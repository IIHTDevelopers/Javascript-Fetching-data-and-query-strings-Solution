...
// Function to check if fetch() is used to make the HTTP request
function checkFetchMethodUsage() {
    let result = 'Pass';
    let feedback = [];
    ...
    // Check if fetch() is used in the code
    if (!studentCode.includes('fetch(')) {
        result = 'Fail';
        feedback.push("You must use fetch() to send an HTTP request.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking fetch() method usage\x1b[0m`);
...
// Function to check if query strings are appended correctly to the URL
function checkQueryStringConstruction() {
    let result = 'Pass';
    let feedback = [];
    ...
    if (!studentCode.includes('?') || !studentCode.includes('&')) {
        result = 'Fail';
        feedback.push("You must construct the URL with query strings using '?' and '&' to separate parameters.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking query string construction\x1b[0m`);
...
// Function to check if the response body is processed correctly (converted to JSON)
function checkResponseBodyHandling() {
    let result = 'Pass';
    let feedback = [];
    ...
    // Check if the response is being converted to JSON
    if (!studentCode.includes('response.json()')) {
        result = 'Fail';
        feedback.push("You must convert the response to JSON using response.json().");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking response body handling (JSON conversion)\x1b[0m`);
...
function gradeAssignment() {
    ...
    testResults.testCaseResults[GUID] = checkFetchMethodUsage();
    testResults.testCaseResults[GUID + '-query-string'] = checkQueryStringConstruction();
    testResults.testCaseResults[GUID + '-response-body'] = checkResponseBodyHandling();
